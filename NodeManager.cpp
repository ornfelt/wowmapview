...
Node NodeManager::getRandomNode(uint32_t mapId) {
    if (nodes.empty()) {
        throw std::runtime_error("Nodes are not loaded yet.");
    }

    // Filter nodes by mapId
    std::vector<Node> filteredNodes;
    std::copy_if(nodes.begin(), nodes.end(), std::back_inserter(filteredNodes),
                 [mapId](const Node& node) { return node.mapId == mapId; });

    if (filteredNodes.empty()) {
        throw std::runtime_error("No nodes available with the specified mapId.");
    }

    // Randomly select one node from filtered nodes
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dist(0, filteredNodes.size() - 1);
